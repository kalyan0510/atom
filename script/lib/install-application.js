...
module.exports = function (packagedAppPath) {
  ...
  if (process.platform === 'darwin') {
    ...
    if (fs.existsSync(apmBinDestinationPath)) {
      console.log(`Removing existing executable at "${apmBinDestinationPath}"`)
      fs.removeSync(apmBinDestinationPath)
    }
    console.log(`Copying atom.sh to "${apmBinDestinationPath}"`)
    fs.symlinkSync(path.join('..', 'share', packagedAppFileName, 'resources', 'app', 'apm', 'node_modules', '.bin', 'apm'), apmBinDestinationPath)
